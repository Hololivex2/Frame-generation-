...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            Log.w(TAG, "Could not register Shizuku permission listener", e)
        }
    ...
    private fun getSystemProperty(name: String): String {
        return try {
            ...
            Log.w(TAG, "Could not read system property: $name", e)
            ""
        }
    }
    ...
    private fun detectGpuInfo(): GpuInfo {
        ...
        // Fallback: try Build props. Some MediaTek ROMs report a backend name
        // such as "meow" here instead of their real Mali GPU.
        if (gpuName == "Unknown") {
            gpuName = getSystemProperty("ro.hardware.egl").ifBlank { Build.HARDWARE }
        }

        // Detect GPU family from both the reported driver and board platform.
        // On many MediaTek phones the EGL property is a vendor codename, not
        // "Mali", so treat a MediaTek platform as Mali unless a real renderer
        // name says otherwise. This avoids a false compatibility warning.
        val chipset = listOf(
            Build.HARDWARE,
            Build.BOARD,
        ...
        val isMediaTek = chipset.contains("mediatek") ||
                Regex("(^|[^a-z])mt[0-9a-z_-]*").containsMatchIn(chipset)
        val isAdreno = gpuName.lowercase().contains("adreno") || chipset.contains("qcom") || chipset.contains("snapdragon")
        val isMali = gpuName.lowercase().contains("mali") || isMediaTek
        if (isMali && (gpuName.equals("meow", ignoreCase = true) || gpuName == "Unknown")) {
            gpuName = "Mali (MediaTek)"
        }
    ...
    private fun checkCompatibility() {
        ...
        compatibilityJob = lifecycleScope.launch {
            ...
            // The previous build only checked permission and never asked for
            // it, so Shizuku stayed red forever even while it was running.
            if (!hasShizuku) requestShizukuPermissionIfNeeded()
        }
    }
    ...
    private fun requestShizukuPermissionIfNeeded() {
        ...
        try {
            ...
            shizukuPermissionRequested = false
            Log.w(TAG, "Unable to request Shizuku permission", e)
        }
    }
